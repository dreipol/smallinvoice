...
def test_add_address():
    a = Address(primary=1, street="Kernstrasse", streetno="60", city="Zurich",
                code="8004", country="CH")
    c = Customer(type=CUSTOMER_TYPE.PRIVATE, gender=CUSTOMER_GENDER.MALE,
                 name="Hanspeter Muster", language="de", addresses=[a])
    client = Client(TEST_API_TOKEN)
...
def test_update_client():
    ...
    a.id = 50341
    c = Customer(type=CUSTOMER_TYPE.PRIVATE, gender=CUSTOMER_GENDER.MALE,
                 name="Hans Muster", language="de", addresses=[a])
    c.id = 49677
    client = Client(TEST_API_TOKEN)
