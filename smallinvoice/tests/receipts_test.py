...
def test_add_receipt():
    ...
    r = Receipt(client_id=24401, client_address_id=24461, currency="CHF",
                date="2013-01-03", language="de", positions=[p])
    client = Client(TEST_API_TOKEN)
...
def test_update_receipt():
    p = Position(type=1, number=2, name="Basisbeitrag", description="Update",
                 cost=2430, unit=3, amount=1)
    p.id = 51090
    r = Receipt(client_id=24401, client_address_id=24461, currency="CHF",
                date="2013-01-03", language="de", positions=[p])
    r.id = 44714
