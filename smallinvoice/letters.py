...
class LetterClient(object):
    ...
    def preview(self, letter_id, page_number, size):
        ...
        return self.client.request_with_method(
            letter_preview % (letter_id, page_number, size,))

    def add(self, client):
        return self.client.request_with_method(add_letter, data=client)["id"]

    def delete(self, letter_id):
        return self.client.request_with_method(delete_letter % (letter_id,),
                                               request_method=REQUEST_METHOD.POST)

    def update(self, letter_id, letter):
        return self.client.request_with_method(update_letter % (letter_id,),
                                               data=letter)

    def email(self, letter_id, letter):
        return self.client.request_with_method(email_letter % (letter_id,),
                                               data=letter)

    def status(self, letter_id, status):
        return self.client.request_with_method(status_letter % (letter_id,),
                                               data=status)